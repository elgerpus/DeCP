...
public class SiftKnnContainer2 implements Serializable {
    public class knnPair implements Comparable<knnPair>, Serializable {
        public long pointID = -1;
    ...
    public SiftKnnContainer2(int k) {
        queryPoint = null;
        b = 1;
        //hasBeenScanned = 1;
        setK(k);
    ...
    public SiftKnnContainer2(int k, int b) {
        queryPoint = null;
        this.b = b;
        //hasBeenScanned = b;
        setK(k);
    ...
    public boolean add(SiftDescriptorContainer point, int index) {
        ...
        if (doScan && queryPoint != null && point.vector.length == queryPoint.vector.length) {
            //int dist = DistanceWithHalt( point );
    ...
    //@Override
    public boolean addNoDuplicateIDs( long id, int distance) {
    ...
    public synchronized boolean add(long id, int distance) {
...
//*/
    ...
    public static SiftKnnContainer2 mergeto1000(SiftKnnContainer2 a, SiftKnnContainer2 b) {
       // create a new knn and add the values from the other input values a and b
        SiftKnnContainer2 ret = new SiftKnnContainer2( 1000 );
    ...
    public static SiftKnnContainer2 mergetosizeofboth(SiftKnnContainer2 a, SiftKnnContainer2 b) {
        // create a new knn and add the values from the other input values a and b
        SiftKnnContainer2 ret = new SiftKnnContainer2( a.getK() + b.getK() );
    ...
    public static SiftKnnContainer2 mergetosize( SiftKnnContainer2 a,
                                                SiftKnnContainer2 b,
                                                int size,
                                                boolean allowDuplicate,
                                                boolean sortFirst) {
        SiftKnnContainer2 ret = new SiftKnnContainer2( size );
        ret.SetQueryPoint(a.getQueryPoint());
    ...
    public static SiftKnnContainer2 mergetosizeoflarger(SiftKnnContainer2 a, SiftKnnContainer2 b) {
        // create a new knn and add the values from the other input values a and b
        int size = a.getK();
        if (b.getK() > size ) {
            size = b.getK();
    ...
    public static SiftKnnContainer2 voteAggregate( SiftKnnContainer2 a, SiftKnnContainer2 b) {
        ...
        SiftKnnContainer2 ret = new SiftKnnContainer2( keys.size() );
        ret.haltScan();
        ret.setcID(qpsum);
        for( Long key : keys) {
    ...
    public static SiftKnnContainer2 shortlist ( SiftKnnContainer2 inn, int maxSize, boolean ascending ) {
        // shrink the size of a k-NN
        SiftKnnContainer2 ret = inn;
        if ( inn.getK() > maxSize ) {
            ret = new SiftKnnContainer2(maxSize);
